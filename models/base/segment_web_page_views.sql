...
{% elif var('segment_page_views_table') is iterable %}
    ...
    {% for table_ref in var('segment_page_views_table', default=[]) %}
        source_{{ loop.index }} as (
            select '{{ table_ref }}' as source_name, *
            from {{ ref(table_ref) }}
        ),
    {% endfor %}

    {#
    The section below takes all of the CTEs generated by the for loop above and then unions them together
    #}

        unioned_sources as (
        {% for table_ref in var('segment_page_views_table', default=[]) %}
            {% if loop.index > 1 %}union all{% endif %}
            select * from source_{{ loop.index }}
        {% endfor %}
        ),
